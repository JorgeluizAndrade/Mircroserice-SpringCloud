...
public class PurchaseService {
	...
	public void dispatchOrder(PurchaseRequest req) {
		...
		InventoryResponse[] inventoryResponse = webClientBuilder.build().get()
		        ...
		        .doOnError(error -> log.error("Erro ao chamar inventário: {}", error.getMessage()))
				.block();
		
		log.info("Resposta do serviço de inventário: {}", Arrays.toString(inventoryResponse));


		boolean allInStock = Arrays.stream(inventoryResponse).peek(resp -> log.info("Resposta {} ", resp)).allMatch(InventoryResponse::isInStock);
